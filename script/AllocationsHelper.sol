...
abstract contract AllocationsHelper is Script {
    ...
    function readAndAllocate(
        ...
        for (uint256 batchIndex = 0; batchIndex < (totalAddresses + BATCH_SIZE - 1) / BATCH_SIZE; batchIndex++) {
            (address[] memory addresses, uint56[] memory amounts, uint256 batchAlloc) = _prepareBatch(
                json,
                allocationKeys,
                batchIndex,
                totalAddresses
    ...
    function _prepareBatch(
    ...
    ) private pure returns (address[] memory addresses, uint56[] memory amounts, uint256 batchAlloc) {
        ...
        amounts = new uint56[](batchLength);
        ...
        for (uint256 i = 0; i < batchLength; i++) {
            ...
            amounts[i] = uint56(vm.parseJsonUint(json, string.concat(".allocations.", addrKey, ".allocation")));
