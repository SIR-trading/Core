...
contract Oracle {
    ...
    function initialize(UniswapFeeTier[] memory uniswapFeeTiers_) external {
        ...
        for (uint48 i = 0; i < NuniswapFeeTiers; i++) {
            uint256 scoreTemp =
                ((uint256(avLiquidity[i]) * uniswapFeeTiers_[i].fee) << 72) / uniswapFeeTiers_[i].tickSpacing;
            if (scoreTemp > score) {
    ...
    function getPrice(address addrCollateralToken) external view returns (bytes16) {
        ...
        // Compute price
        bytes16 priceFP_ = FloatingPoint.fromInt(oracleData.aggLogPrice / int256(uint256(oracleData.period))).mul(
            _LOG2_OF_TICK_FP
        ).pow_2();
    ...
    function updatePriceMemory(address addrCollateralToken) external returns (bytes16) {
        ...
        if (oracleData.increaseCardinality) {
            ...
            OracleData memory oracleDataOther =
                _getUniswapOracleData(uniswapFeeTiers[_indexNextProbedFeeTier].fee, false);

            // If the probed fee tier is better, continue extending its cardinality if needed
            uint256 scoreCurrentFeeTier = (
                (uint256(oracleData.avLiquidity) * uniswapFeeTiers[_indexCurrentFeeTier].fee) << 72
            ) / uniswapFeeTiers[_indexCurrentFeeTier].tickSpacing;
            uint256 scoreNextProbedFeeTier = (
                (
                    uint256(oracleDataOther.avLiquidity) // Better higher in-range liquidity
                        * uniswapFeeTiers[_indexNextProbedFeeTier].fee
                ) << 72
            ) // Better higher fee because it is more expensive to manipulate
    ...
    function _newFeeTier(UniswapFeeTier memory feeTier) internal returns (bool) {
        ...
        address addrPool = UniswapPoolAddress.computeAddress(
            Addresses.ADDR_UNISWAPV3_FACTORY, UniswapPoolAddress.getPoolKey(TOKEN_B, TOKEN_A, feeTier.fee)
        );
        ...
        feeTier.maxPriceAttenPerSec =
            FloatingPoint.ONE.sub(FloatingPoint.divu(feeTier.fee, 0.5 * 10 ** 6)).pow(ONE_DIV_SIXTY); // 5 blocks of 12s = 60s
    ...
    function _getUniswapOracleData(uint24 fee, bool instantData) private view returns (OracleData memory oracleData) {
        ...
        try uniswapV3Pools[fee].observe(interval) returns (
            int56[] memory tickCumulatives_, uint160[] memory liquidityCumulatives_
        ) {
            ...
            bool initialized;
            (blockTimestampOldest,,, initialized) =
                uniswapV3Pools[fee].observations((observationIndex + 1) % cardinalityNow);
