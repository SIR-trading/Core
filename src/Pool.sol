...
contract Pool is MAAM, PoolStructs {
    ...
    /**
        ...
        @dev All view functions are outsourced to POOL_LOGIC
     */
    function mintTEA() external returns (uint256) {
        ...
        (PoolStructs.Reserves memory reservesPre, uint256 amountTEA, uint256 feeToPOL) = POOL_LOGIC.quoteMint(
            state_,
            _LEVERAGE_TIER,
            price,
    ...
    function mintAPE() external returns (uint256) {
        ...
        PoolStructs.State memory state_ = state;
        (PoolStructs.Reserves memory reservesPre, uint256 amountAPE, uint256 feeToPOL) = POOL_LOGIC.quoteMint(
            state_,
            _LEVERAGE_TIER,
            price,
    ...
    function burnTEA(uint256 amountTEA) external returns (uint256) {
        ...
        PoolStructs.State memory state_ = state;
        (PoolStructs.Reserves memory reservesPre, uint256 collateralWithdrawn, uint256 feeToPOL) = POOL_LOGIC.quoteBurn(
            state_,
            _LEVERAGE_TIER,
            price,
            _TEA_TOKEN.totalSupply(),
    ...
    function mintMAAM() external returns (uint256) {
        ...
        PoolStructs.State memory state_ = state;
        (uint256 LPReservePre, uint256 collateralDeposited) = POOL_LOGIC.quoteMintMAAM(
            state_,
            _LEVERAGE_TIER,
            price,
            _COLLATERAL_TOKEN
    ...
    function burnMAAM(uint256 amountMAAM) external returns (uint256) {
        ...
        PoolStructs.State memory state_ = state;
        uint256 LPReservePre = POOL_LOGIC.quoteBurnMAAM(state_, _LEVERAGE_TIER, price, amountMAAM);
    ...
    function totalSupply() public view override returns (uint256) {
        ...
        PoolStructs.Reserves memory reserves = POOL_LOGIC.getReserves(state, _LEVERAGE_TIER, price);
    ...
    //     )
    // {
    //     return POOL_LOGIC.priceStabilityRange(state, _LEVERAGE_TIER);
    // }
    ...
    //     (bytes16 leverageRatio, bytes16 collateralizationFactor) = POOL_LOGIC.calculateRatios(_LEVERAGE_TIER);
    ...
    /**
        @notice Multisig/DAOFees withdraws collected POOL_LOGIC
     */
    function withdrawDAOFees() external returns (uint256 DAOFees) {
        require(msg.sender == POOL_LOGIC.SYSTEM_CONTROL());
