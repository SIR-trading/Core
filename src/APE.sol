...
contract APE is SyntheticToken {
    ...
    function _generateName(
        ...
        if (leverageTier >= 0) return Strings.toString(1 + 2**uint256(int256(leverageTier)));
        else {
            ...
            for (uint256 i = 0; i < decimalPartStr.length; i++) decimalPartStr[i] = nonDecimalPoinStr[i + 2];
